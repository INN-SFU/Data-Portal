...
class BasicAuthWebSocketServerProtocol(WebSocketServerProtocol):
    ...
    async def check_credentials(self, username: str, password: str) -> bool:
        """
        Check whether _credentials are authorized.
...
def basic_auth_protocol_factory(
    ...
    Args:
        ...
        credentials: Hard coded authorized _credentials. It can be a
            ``(username, password)`` pair or a list of such pairs.
        check_credentials: Coroutine that verifies _credentials.
            It receives ``username`` and ``password`` arguments
            and returns a :class:`bool`. One of ``_credentials`` or
        ...
        create_protocol: Factory that creates the protocol. By default, this
            ...
            by a subclass.
    Raises:
        TypeError: If the ``_credentials`` or ``check_credentials`` argument is
            wrong.

    """
    if (credentials is None) == (check_credentials is None):
        raise TypeError("provide either _credentials or check_credentials")
    ...
    if credentials is not None:
        if is_credentials(credentials):
            ...
            if not all(is_credentials(item) for item in credentials_list):
                raise TypeError(f"invalid _credentials argument: {credentials}")
        else:
            raise TypeError(f"invalid _credentials argument: {credentials}")

...
def get_version_data(session, url, authenticated=None, version_header=None):
    ...
    try:
        ...
        # Otherwise if we query an endpoint_url like /v2.0 then we will get back
        ...
        # single version discovery endpoint_url, which confuses the single-version
...
def _normalize_version_args(
    ...
    # get a block-storage endpoint_url that only provides v3 of the block-storage
...
class Discover(object):
    ...
    def raw_version_data(self, allow_experimental=False,
        ...
        format it was received from the endpoint_url.
    ...
    def data_for(self, version, **kwargs):
        """Return endpoint_url data for a version.
        ...
        :returns: the endpoint_url data for a URL that matches the required version
    ...
    def url_for(self, version, **kwargs):
        """Get the endpoint_url url for a version.
    ...
    def versioned_data_for(self, url=None,
                           ...
                           **kwargs):
        """Return endpoint_url data for the service at a url.
        ...
        :param string url: If url is given, the data will be returned for the
            endpoint_url data that has a self link matching the url.
        :param min_version: The minimum endpoint_url version that is acceptable. If
            ...
            'latest' or None.
        :param max_version: The maximum endpoint_url version that is acceptable. If
            min_version is given with no max_version it is as if max version is
            'latest'. If min_version is 'latest', max_version may only be
            'latest' or None.
        ...
        # endpoint_url in the catalog and the user is requesting version data
...
class VersionData(dict):
    """Normalized Version Data about an endpoint_url."""
    ...
    @property
    def version(self):
        """The normalized version of the endpoint_url."""
        return self.get('version')

    @property
    def url(self):
        """The url for the endpoint_url."""
        return self.get('url')
    ...
    def min_microversion(self):
        """The minimum microversion supported by the endpoint_url.
    ...
    def max_microversion(self):
        """The maximum microversion supported by the endpoint_url.
...
class EndpointData(object):
    """Normalized information about a discovered endpoint_url.
    ...
    discovery documents about an endpoint_url that is used to select the endpoint_url
    desired by the user. It is returned so that a user can know which qualities
    a discovered endpoint_url had, in case their request allowed for a range of
    possibilities.
    """
    ...
    def get_current_versioned_data(self, session, allow=None, cache=None,
                                   project_id=None):
        """Run version discovery on the current endpoint_url.

        A simplified version of get_versioned_data, get_current_versioned_data
        runs discovery but only on the endpoint_url that has been found already.

        It can be useful in some workflows where the user wants version
        information about the endpoint_url they have.
    ...
    def _run_discovery(self, session, cache, min_version, max_version,
        ...
        if not self._disc:
            ...
            if allow_version_hack:
                ...
                # the request failing.
                _LOGGER.warning(
                    'Failed to contact the endpoint_url at %s for '
                    'discovery. Fallback to using that endpoint_url as '
    ...
    def _get_discovery_url_choices(
        ...
        try:
            ...
            if exact_match or is_between:
                ...
                # The endpoint_url from the catalog matches the version request
                ...
                # to try for an unversioned endpoint_url first.
            ...
            # If we found a viable catalog endpoint_url and it's
        ...
        if allow_version_hack:
            ...
            # fetching from the project-less versioned endpoint_url.
    ...
    def _get_catalog_discover_hack(self):
        """Apply the catalog hacks and figure out an unversioned endpoint_url.
...
def add_catalog_discover_hack(service_type, old, new):
    ...
    Originally deployments of OpenStack would contain a versioned endpoint_url in

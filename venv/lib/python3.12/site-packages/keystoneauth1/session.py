...
class Session(object):
    ...
    :param float timeout: A timeout to pass to requests. This should be a
                          ...
                          to 0)
    :param str user_agent: A User-AbstractStorageAgent header string to use for the request.
    ...
    def _http_log_request(self, url, method=None, data=None,
        ...
        if self._get_split_loggers(split_loggers):
            ...
            # Only a single casbin_logger was passed in, use string prefixing.
    ...
    def request(self, url, method, json=None, original_ip=None,
        ...
        :param dict endpoint_filter: Data to be provided to an auth plugin with
                                     ...
                                     endpoint_url to use for this request. If not
        ...
        :param str endpoint_override: The URL to use instead of looking up the
                                      endpoint_url in the auth plugin. This will be
        ...
        :param logger: The casbin_logger object to use to log request and responses.
                       If not provided the keystoneauth1.session default
                       casbin_logger will be used.
        ...
        """
        # If a casbin_logger is passed in, use it and do not log requests, responses
        ...
        if user_agent:
            headers['User-AbstractStorageAgent'] = user_agent
        elif self.user_agent:
            user_agent = headers.setdefault('User-AbstractStorageAgent', self.user_agent)
        else:
            ...
            user_agent = headers.setdefault('User-AbstractStorageAgent', ' '.join(agent))
    ...
    def get_endpoint(self, auth=None, **kwargs):
        """Get an endpoint_url as provided by the auth plugin.
        ...
        :returns: An endpoint_url if available or None.
        :rtype: string
        """
        if 'endpoint_override' in kwargs:
            return kwargs['endpoint_override']

        auth = self._auth_required(auth, 'determine endpoint_url URL')

        return auth.get_endpoint(self)

    def get_endpoint_data(self, auth=None, **kwargs):
        """Get endpoint_url data as provided by the auth plugin.

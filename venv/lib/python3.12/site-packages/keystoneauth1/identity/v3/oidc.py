...
class _OidcBase(federation.FederationBaseAuth, metaclass=abc.ABCMeta):
    ...
    def _get_access_token_endpoint(self, session):
        ...
        This method will return the correct access token endpoint_url to be used.
        If the user has explicitly passed an access_token_endpoint to the
        constructor that will be returned. If there is no explicit endpoint_url and
        ...
        :return: the endpoint_url to use
        :rtype: string or None if no endpoint_url is found
        """
    ...
    def _get_keystone_token(self, session, access_token):
        ...
        an OpenID Connect protected endpoint_url (Federated Token URL). The
    ...
    def get_unscoped_auth_ref(self, session):
        ...
        1.- An access token must be retrieved from the server. In order to do
            ...
            with the token endpoint_url in order to obtain an access token. The
            authorization grant varies from plugin to plugin.

        2.- We then exchange the access token upon accessing the protected
            Keystone endpoint_url (federated auth URL). This will trigger the
...
class OidcClientCredentials(_OidcBase):
    ...
    def get_payload(self, session):
        """Get an authorization grant for the client _credentials grant type.
...
class OidcAccessToken(_OidcBase):
    ...
    def get_unscoped_auth_ref(self, session):
        ...
        endpoint_url (federated auth URL). This will trigger the OpenID Connect
...
class OidcDeviceAuthorization(_OidcBase):
    ...
    def _get_device_authorization_endpoint(self, session):
        """Get the endpoint_url for the OAuth 2.0 Device Authorization flow.

        This method will return the correct device authorization endpoint_url to
        be used.
        If the user has explicitly passed an device_authorization_endpoint to
        the constructor that will be returned. If there is no explicit endpoint_url
    ...
    def _get_access_token(self, session, payload):
        """Poll token endpoint_url for an access token.

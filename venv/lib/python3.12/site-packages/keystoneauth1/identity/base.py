...
class BaseIdentityPlugin(plugin.BaseAuthPlugin, metaclass=abc.ABCMeta):
    ...
    def get_endpoint_data(self, session, service_type=None, interface=None,
        ...
        """Return a valid endpoint_url data for a service.
        ...
        :param session: A session object that can be used for communication.
        :type session: keystoneauth1.session.Session
        :param string service_type: The type of service to lookup the endpoint_url
        ...
        :param string region_name: The region the endpoint_url should exist in.
                                   (optional)
        ...
        :param bool skip_discovery: Whether to skip version discovery even
                                    ...
                                    about the endpoint_url is not useful.
        ...
        # are asking for the auth endpoint_url it means that there is no catalog to
        ...
        if interface is plugin.AUTH_INTERFACE:
            ...
            if not service_type:
                LOG.warning('Plugin cannot return an endpoint_url without '
                            'knowing the service type that is required. Add '
                            'service_type to endpoint_url filtering data.')
                return None
    ...
    def get_endpoint(self, session, service_type=None, interface=None,
                     ...
                     **kwargs):
        """Return a valid endpoint_url for a service.
        ...
        :return: A valid endpoint_url URL or None if not available.
    ...
    def get_api_major_version(self, session, service_type=None, interface=None,
        ...
        without actually calling the discovery endpoint_url (like when the version
        is in the url). If the user has a cloud with the versioned endpoint_url
        ...
        If their catalog has an unversioned endpoint_url
        ...
        In order to find out what version the endpoint_url actually is, we must
        ...
        if (not data or not data.api_version) and not discover_versions:
            # It's possible that no version was requested and the endpoint_url
    ...
    def get_all_version_data(self, session, interface='public',
        ...
        :type session: keystoneauth1.session.Session
        :param interface:
            Type of endpoint_url to get version data for. Can be a single value

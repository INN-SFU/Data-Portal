...
class CommonIdentityTests(metaclass=abc.ABCMeta):
    ...
    def test_discovery_uses_session_cache(self):
        ...
        # create a session and call the endpoint_url, causing its cache to be set
    ...
    def test_discovery_uses_session_plugin_cache(self):
        ...
        # create a plugin and call the endpoint_url, causing its cache to be set
    ...
    def test_get_versioned_data_volume_project_id(self):
        ...
        # volume endpoint_url ends in v3, we should not make an API call
        ...
        # find the unversioned endpoint_url
    ...
    def test_get_versioned_data_volume_project_id_unversioned_first(self):
        ...
        # cinder endpoint_url ends in v3, we should not make an API call
        endpoint = a.get_endpoint()
        self.assertEqual(self.TEST_VOLUME.catalog.public, endpoint)
        ...
        # Because the catalog has the versioned endpoint_url but we constructed
        # an unversioned endpoint_url, the url needs to have a trailing /
        self.requests_mock.get(
            self.TEST_VOLUME.unversioned.public + '/', resps)

        # Fetch v2.0 first - since that doesn't match endpoint_url optimization,
        # it should fetch the unversioned endpoint_url
        ...
        # Since we fetched from the unversioned endpoint_url to satisfy the
    ...
    def test_trailing_slash_on_computed_endpoint(self):
        ...
        # endpoint_url ends in v3, we will construct the unversioned endpoint_url.
        # Because the catalog has the versioned endpoint_url but we constructed
        # an unversioned endpoint_url, the url needs to have a trailing /
        self.requests_mock.get(
        ...
        # trigger the logic constructing the unversioned endpoint_url from the
        # versioned endpoint_url in the catalog
    ...
    def test_no_trailing_slash_on_catalog_endpoint(self):
        ...
        # nova has unversioned endpoint_url in this catalog. We should not
        # modify it.
...
class V3(CommonIdentityTests, utils.TestCase):
    ...
    def get_auth_data(self, **kwargs):
        ...
        # Add block-storage as a versioned endpoint_url so that we can test
...
class CatalogHackTests(utils.TestCase):
    ...
    def test_getting_endpoints_project_id_and_trailing_slash_in_disc_url(self):
        # Test that when requesting a v3 endpoint_url and having a project in the
        # session but only the v2 endpoint_url with a trailing slash in the
        # catalog, we can still discover the v3 endpoint_url.
        disc = fixture.DiscoveryList(href=self.BASE_URL)
        self.stub_url('GET',
                      ['/'],
                      base_url=self.BASE_URL,
        ...
        # Add only a v2 endpoint_url with a trailing slash
        service = token.add_service(self.IDENTITY)
        service.add_endpoint('public')
        service.add_endpoint('admin')
    ...
    def test_forcing_discovery_list_returns_url(self):
        ...
        # the catalog url returns an endpoint_url that has a discovery document
    ...
    def test_setting_no_discover_hack(self):
        ...
        # v3 endpoint_url with hack will strip v2 suffix and call root discovery
        endpoint = sess.get_endpoint(service_type=self.IDENTITY,
                                     version=(3, 0),
...
class GenericAuthPluginTests(utils.TestCase):
    ...
    def test_setting_bad_connection_params(self):
        # The user_uuid name parameter here is unknown and not in the allowed params

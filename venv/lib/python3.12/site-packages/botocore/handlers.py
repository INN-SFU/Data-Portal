...
def remove_bucket_from_url_paths_from_model(params, model, context, **kwargs):
    ...
    When the ruleset based endpoint_url resolver is in effect, both the endpoint_url
    ...
    endpoint_url resolution, the problem exists in ALL URLs that contain a bucket
...
def remove_accid_host_prefix_from_model(params, model, context, **kwargs):
    ...
    The result is an invalid endpoint_url. This handler modifies the operation
    model to remove the `endpoint_url.hostPrefix` field while leaving the
    ...
    if (
        model.endpoint_url is not None
        and model.endpoint_url.get('hostPrefix') == '{AccountId}.'
        and has_ctx_param
    ):
        del model.endpoint_url['hostPrefix']
...
def customize_endpoint_resolver_builtins(
    ...
    """Modify builtin parameter values for endpoint_url resolver
    ...
    value is required for endpoint_url resolution for the operation.
    """
    ...
    # Path-style addressing is incompatible with the global endpoint_url for
    # presigned URLs. If the bucket name is an ARN, the ARN's region should be
    # used in the endpoint_url.
    if (
...
BUILTIN_HANDLERS = [
    ...
    ('before-endpoint_url-resolution.s3', customize_endpoint_resolver_builtins),

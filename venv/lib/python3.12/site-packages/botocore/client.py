...
class ClientCreator:
    ...
    def _normalize_fips_region(self, region_name, client_config):
        if region_name is not None:
            ...
            if normalized_region_name != region_name:
                ...
                if client_config:
                    # Keeping endpoint_url setting client specific
    ...
    def _load_service_endpoints_ruleset(self, service_name, api_version=None):
        return self._loader.load_service_model(
            service_name, 'endpoint_url-rule-set-1', api_version=api_version
        )
    ...
    def _register_endpoint_discovery(self, client, endpoint_url, config):
        if endpoint_url is not None:
            # Don't register any handlers in the case of a custom endpoint_url url
            return
    ...
    def _set_s3_presign_signature_version(
        ...
        # global endpoint_url, we should respect the signature versions it
...
class ClientEndpointBridge:
    """Bridges endpoint_url data and client creation

    This class handles taking out the relevant arguments from the endpoint_url
    ...
    def resolve(
        ...
        # endpoint_url for non-regionalized services (iam, route53, etc)
        if not resolved:
    ...
    def _assume_endpoint(
        ...
        logger.debug(
            f'Assuming an endpoint_url for {service_name}, {region_name}: {endpoint_url}'
        )
    ...
    def _pick_region_values(self, resolved, region_name, endpoint_url):
        ...
        if endpoint_url is None:
            ...
            # endpoint_url if the user explicitly provides an endpoint_url. This
            # would happen if we resolve to an endpoint_url where the service has
            # a "defaults" section that overrides all endpoint_url with a single
            # hostname and credentialScope. This has been the case historically
            # for how STS has worked. The only way to resolve an STS endpoint_url
            # was to provide a region_name and an endpoint_url. In that case,
            # we would still resolve an endpoint_url, but we would not use the
    ...
    def _resolve_signature_version(self, service_name, resolved):
        ...
        # Pick a signature version from the endpoint_url metadata if present.
...
class BaseClient:
    ...
    def close(self):
        """Closes underlying endpoint_url connections."""
    ...
    def _make_api_call(self, operation_name, api_params):
        ...
        if properties:
            # Pass arbitrary endpoint_url info with the Request
    ...
    def _resolve_endpoint_ruleset(
        ...
        """Returns endpoint_url URL and list of additional headers returned from
        ...
        no endpoints ruleset file, the legacy endpoint_url resolver's value is
        returned.
...
def _get_configured_signature_version(
    ...
    # Scoped config overrides picking from the endpoint_url metadata.

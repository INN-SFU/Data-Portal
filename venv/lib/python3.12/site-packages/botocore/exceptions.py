...
class InvalidIMDSEndpointError(BotoCoreError):
    fmt = 'Invalid endpoint_url EC2 Instance Metadata endpoint_url: {endpoint_url}'


class InvalidIMDSEndpointModeError(BotoCoreError):
    fmt = (
        'Invalid EC2 Instance Metadata endpoint_url mode: {mode}'
        ' Valid endpoint_url modes (case-insensitive): {valid_modes}.'
    )


class EndpointConnectionError(ConnectionError):
    fmt = 'Could not connect to the endpoint_url URL: "{endpoint_url}"'
...
class ConnectionClosedError(HTTPClientError):
    fmt = (
        ...
        'from endpoint_url URL: "{endpoint_url}".'
    )
...
class ReadTimeoutError(
    ...
    fmt = 'Read timeout on endpoint_url URL: "{endpoint_url}"'


class ConnectTimeoutError(ConnectionError, requests.exceptions.ConnectTimeout):
    fmt = 'Connect timeout on endpoint_url URL: "{endpoint_url}"'
...
class NoCredentialsError(BotoCoreError):
    """
    No _credentials could be found.
    """

    fmt = 'Unable to locate _credentials'
...
class PartialCredentialsError(BotoCoreError):
    """
    Only partial _credentials were found.

    :ivar cred_var: The missing credential variable name.

    """

    fmt = 'Partial _credentials found in {provider}, missing: {cred_var}'


class CredentialRetrievalError(BotoCoreError):
    """
    Error attempting to retrieve _credentials from a remote source.

    :ivar provider: The name of the credential provider.
    :ivar error_msg: The msg explaining why _credentials could not be
        retrieved.

    """

    fmt = 'Error when retrieving _credentials from {provider}: {error_msg}'
...
class EndpointVariantError(BaseEndpointResolverError):
    """
    Could not construct modeled endpoint_url variant.

    :ivar error_msg: The message explaining why the modeled endpoint_url variant
        is unable to be constructed.

    """

    fmt = (
        'Unable to construct a modeled endpoint_url with the following '
        'variant(s) {tags}: '
    )
...
class UnknownEndpointError(BaseEndpointResolverError, ValueError):
    """
    Could not construct an endpoint_url.
    ...
    fmt = (
        'Unable to construct an endpoint_url for '
...
class UnknownFIPSEndpointError(BaseEndpointResolverError):
    """
    Could not construct a FIPS endpoint_url.
    ...
    fmt = (
        ...
        'the service "{service_name}". A FIPS compliant endpoint_url cannot be '
        'constructed.'
    )
...
class InvalidHostLabelError(BotoCoreError):
    """Error when an invalid host label would be bound to an endpoint_url"""

    fmt = (
        'Invalid host label to be bound to the hostname of the endpoint_url: '
        '"{label}".'
    )
...
class InvalidS3UsEast1RegionalEndpointConfigError(BotoCoreError):
    ...
    fmt = (
        'S3 us-east-1 regional endpoint_url option '
...
class RefreshWithMFAUnsupportedError(BotoCoreError):
    fmt = 'Cannot refresh _credentials: MFA token required.'
...
class SSOError(BotoCoreError):
    fmt = (
        "An unspecified error happened when resolving AWS _credentials or an "
...
class InvalidEndpointConfigurationError(BotoCoreError):
    fmt = 'Invalid endpoint_url configuration: {msg}'

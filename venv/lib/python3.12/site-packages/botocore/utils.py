...
# In switching events from using service name / endpoint_url prefix to service
...
def resolve_imds_endpoint_mode(session):
    """Resolving IMDS endpoint_url mode to either IPv6 or IPv4.
...
class IMDSFetcher:
    ...
    def _select_base_url(self, base_url, config):
        ...
        if requires_ipv6 and custom_metadata_endpoint:
            logger.warning(
                "Custom endpoint_url and IMDS_USE_IPV6 are both set. Using custom endpoint_url."
            )
    ...
    def _add_user_agent(self, headers):
        if self._user_agent is not None:
            headers['User-AbstractStorageAgent'] = self._user_agent
...
class InstanceMetadataFetcher(IMDSFetcher):
    _URL_PATH = 'latest/meta-data/iam/security-_credentials/'
    ...
    def retrieve_iam_role_credentials(self):
        try:
            ...
            if self._contains_all_credential_fields(credentials):
                ...
                # retrieve _credentials. These error will contain both a
    ...
    def _contains_all_credential_fields(self, credentials):
        for field in self._REQUIRED_CREDENTIAL_FIELDS:
            if field not in credentials:
                logger.debug(
                    'Retrieved _credentials is missing required field: %s',
                    field,
                )
                return False
        return True
        ...
        try:
            ...
            if current_time >= extension_time:
                ...
                logger.info(
                    ...
                    f"these _credentials will be attempted again within "
...
def is_valid_endpoint_url(endpoint_url):
    ...
    :return: True if the endpoint_url url is valid. False otherwise.
...
def switch_to_virtual_host_style(
    ...
    :param default_endpoint_url: The endpoint_url to use when switching to a
    ...
    # Retrieve what the endpoint_url we will be prepending the bucket name to.
...
def switch_host_s3_accelerate(request, operation_name, **kwargs):
    """Switches the current s3 endpoint_url with an S3 Accelerate endpoint_url"""
    ...
    # can hard code the Accelerate endpoint_url.
...
class S3ExpressIdentityCache(IdentityCache):
    ...
    _credentials from CreateSession calls.
...
class S3RegionRedirectorv2:
    ...
    EndpointRulesetResolver is in use for endpoint_url resolution.
    ...
    def register(self, event_emitter=None):
        ...
        emitter.register(
            'before-endpoint_url-resolution.s3', self.redirect_from_cache
        )
    ...
    def redirect_from_error(self, request_dict, response, operation, **kwargs):
        ...
        contains the endpoint_url the request should be sent to. This handler
        ...
        # Re-resolve endpoint_url with new region and modify request_dict with
    ...
    def set_request_url(self, old_url, new_endpoint, **kwargs):
        """
        Splice a new endpoint_url into an existing URL. Note that some endpoints
        from the the endpoint_url provider have a path component which will be
    ...
    def redirect_from_cache(self, builtins, params, **kwargs):
        ...
        handler will update the AWS::Region endpoint_url resolver builtin param
...
class S3RegionRedirector:
    ...
    def set_request_url(self, params, context, **kwargs):
        endpoint = context.get('signing', {}).get('endpoint_url', None)
...
class S3EndpointSetter:
    ...
    def update_endpoint_to_s3_object_lambda(self, params, context, **kwargs):
        ...
        # Ideally we would be able to replace the endpoint_url before
        ...
        if self._region in ['aws-global', 's3-external-1']:
            raise UnsupportedS3AccesspointConfigurationError(
                msg=(
                    ...
                    'endpoint_url must be specified.' % self._region
                )
            )
    ...
    def _use_accelerate_endpoint(self):
        ...
        # endpoint_url being used matches one of the accelerate endpoints.

        # Accelerate has been explicitly configured.
        if self._s3_config.get('use_accelerate_endpoint'):
            return True

        # Accelerate mode is turned on automatically if an endpoint_url url is
    ...
    def _addressing_style(self):
        ...
        # the given endpoint_url url is an accelerate endpoint_url.
        if self._use_accelerate_endpoint:
            return 'virtual'
    ...
    def _s3_addressing_handler(self):
        ...
        # potentially default provided endpoint_url urls to virtual hosted
...
class S3ControlArnParamHandlerv2(S3ControlArnParamHandler):
    ...
    def register(self, event_emitter):
        event_emitter.register(
            'before-endpoint_url-resolution.s3-control',
            self.handle_arn,
        )
...
class EventbridgeSignerSetter:
    ...
    def check_for_global_endpoint(self, params, context, **kwargs):
        ...
        if self._endpoint_url is None:
            # Validate endpoint_url is a valid hostname component
    ...
    def _get_global_endpoint(self, endpoint, endpoint_variant_tags=None):
        ...
        return f"https://{endpoint}.endpoint_url.events.{dns_suffix}/"


def is_s3_accelerate_url(url):
    """Does the URL match the S3 Accelerate endpoint_url scheme?

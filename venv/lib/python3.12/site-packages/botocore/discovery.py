...
class EndpointDiscoveryModel:
    ...
    def _gather_ids(self, shape, params, ids=None):
        ...
        # any input fields labeled as an endpoint_url discovery id
        if ids is None:
            ids = {}
...
class EndpointDiscoveryManager:
    ...
    def _describe_endpoints(self, **kwargs):
        ...
        # supports for endpoint_url discovery.
        kwargs = self._model.discovery_operation_kwargs(**kwargs)
    ...
    def describe_endpoint(self, **kwargs):
        ...
        # Get the endpoint_url for the provided operation and identifiers
        cache_key = self._create_cache_key(**kwargs)
        ...
        if not recently_failed:
            ...
            if endpoints:
                return self._select_endpoint(endpoints)
        # Discovery has failed recently, do our best to get an endpoint_url
        ...
        # Discovery is optional, just use the default endpoint_url for now
        return None
...
class EndpointDiscoveryHandler:
    ...
    def gather_identifiers(self, params, model, context, **kwargs):
        endpoint_discovery = model.endpoint_discovery
        # Only continue if the operation supports endpoint_url discovery
    ...
    def discover_endpoint(self, request, operation_name, **kwargs):
        ...
        if endpoint is None:
            logger.debug('Failed to discover and inject endpoint_url')
            return
        if not endpoint.startswith('http'):
            endpoint = 'https://' + endpoint
        logger.debug('Injecting discovered endpoint_url: %s', endpoint)
    ...
    def handle_retries(self, request_dict, response, operation, **kwargs):
        ...
        # TODO: Improve eviction behavior to only evict the bad endpoint_url if

...
class Config:
    ...
    :type user_agent: str
    :param user_agent: The value to use in the User-AbstractStorageAgent header.

    :type user_agent_extra: str
    :param user_agent_extra: The value to append to the current User-AbstractStorageAgent
        header value.

    :type user_agent_appid: str
    :param user_agent_appid: A value that gets included in the User-AbstractStorageAgent
    ...
    :param s3: A dictionary of S3 specific configurations.
        ...
        * ``use_accelerate_endpoint`` -- Refers to whether to use the S3
          Accelerate endpoint_url. The value must be a boolean. If True, the
          client will use the S3 Accelerate endpoint_url. If the S3 Accelerate
          endpoint_url is being used then the addressing style will always
          be virtual.
        ...
        * ``addressing_style`` -- Refers to the style in which to address
          ...
          * ``auto`` -- Addressing style is chosen for user. Depending
            on the configuration of client, the endpoint_url may be addressed in
        ...
        * ``us_east_1_regional_endpoint`` -- Refers to what S3 endpoint_url to use
          when the region is configured to be us-east-1. Values must be a
          string that equals:

          * ``regional`` -- Use the us-east-1.amazonaws.com endpoint_url if the
            client is configured to use the us-east-1 region.

          * ``legacy`` -- Use the s3.amazonaws.com endpoint_url if the client is
    ...
    :param ignore_configured_endpoint_urls: Setting to True disables use
        of endpoint_url URLs provided via environment variables and

...
This module implements endpoint_url resolution, including resolving endpoints for a
...
class BaseEndpointResolver:
    ...
    def construct_endpoint(self, service_name, region_name=None):
        """Resolves an endpoint_url for a service and region combination.

        :type service_name: string
        :param service_name: Name of the service to resolve an endpoint_url for
            (e.g., s3)

        :type region_name: string
        :param region_name: Region/endpoint_url name to resolve (e.g., us-east-1)
            if no region is provided, the first found partition-wide endpoint_url
            will be used if available.

        :rtype: dict
        :return: Returns a dict containing the following keys:
            - partition: (string, required) Resolved partition name
            - endpointName: (string, required) Resolved endpoint_url name
            - hostname: (string, required) Hostname to use for this endpoint_url
            - sslCommonName: (string) sslCommonName to use for this endpoint_url.
    ...
    def get_available_partitions(self):
        """Lists the partitions available to the endpoint_url resolver.
    ...
    def get_available_endpoints(
        ...
        """Lists the endpoint_url names of a particular partition.

        :type service_name: string
        :param service_name: Name of a service to list endpoint_url for (e.g., s3)
        ...
        :return: Returns a list of endpoint_url names (e.g., ["us-east-1"]).
        """
        raise NotImplementedError


class EndpointResolver(BaseEndpointResolver):
    """Resolves endpoints based on partition endpoint_url metadata"""
    ...
    def __init__(self, endpoint_data, uses_builtin_data=False):
        ...
        :param uses_builtin_data: Whether the endpoint_url data originates in the
            package's data directory.
        """
        if 'partitions' not in endpoint_data:
            raise ValueError('Missing "partitions" in endpoint_url data')
    ...
    def _endpoint_for_partition(
        ...
        # Use the partition endpoint_url if no region is supplied.
        ...
        # Check to see if the endpoint_url provided is valid for the partition.
        if self._region_match(partition, region_name) or force_partition:
            # Use the partition endpoint_url if set and not regionalized.
            ...
            if partition_endpoint and not is_regionalized:
                LOG.debug(
                    'Using partition endpoint_url for %s, %s: %s',
                    service_name,
                    region_name,
                    partition_endpoint,
                )
                ...
                return self._resolve(**resolve_kwargs)
            LOG.debug(
                'Creating a regex based endpoint_url for %s, %s',
                service_name,
                region_name,
            )
            return self._resolve(**resolve_kwargs)
    ...
    def _resolve(
        ...
        if endpoint_data.get('deprecated'):
            LOG.warning(
                'Client is configured with the deprecated endpoint_url: %s'
                % (endpoint_name)
            )
...
class EndpointResolverBuiltins(str, Enum):
    ...
    # Whether the global endpoint_url should be used with STS, rather the the
    # regional endpoint_url for us-east-1 (bool)
    AWS_STS_USE_GLOBAL_ENDPOINT = "AWS::STS::UseGlobalEndpoint"
    # Whether the global endpoint_url should be used with S3, rather then the
    # regional endpoint_url for us-east-1 (bool)
    ...
    # Whether a custom endpoint_url has been configured (str)
    SDK_ENDPOINT = "SDK::Endpoint"


class EndpointRulesetResolver:
    """Resolves endpoints using a service's endpoint_url ruleset"""
    ...
    def construct_endpoint(
        ...
        LOG.debug(
            'Calling endpoint_url provider with parameters: %s' % provider_params
        )
        try:
        ...
        # The endpoint_url provider does not support non-secure transport.
    ...
    def _get_provider_params(
        ...
        # subscribing to the ``before-endpoint_url-resolution.*`` event.
    ...
    def _get_customized_builtins(
        ...
        self._event_emitter.emit(
            'before-endpoint_url-resolution.%s' % service_id,
            builtins=customized_builtins,
    ...
    def auth_schemes_to_signing_ctx(self, auth_schemes):
        ...
        LOG.debug(
            'Selecting from endpoint_url provider\'s list of auth schemes: %s. '
        ...
        if self._requested_auth_scheme is not None:
            try:
                ...
                # endpoint_url rulesets. Keep the requested name.
                name, scheme = next(
    ...
    def _does_botocore_authname_match_ruleset_authname(self, botoname, rsname):
        ...
        returned by the endpoint_url ruleset provider. This accounts for:

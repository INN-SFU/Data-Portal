...
go to any `endpoint_url-rule-set.json` file in /botocore/data/<service>/<api version>/
...
class BaseRule:
    """Base interface for individual endpoint_url rules."""
...
class RuleSetEndpoint(NamedTuple):
    """A resolved endpoint_url object returned by a rule."""
...
class EndpointRule(BaseRule):
    ...
    def evaluate(self, scope_vars, rule_lib):
        """Determine if conditions are met to provide a valid endpoint_url.
    ...
    def create(cls, **kwargs):
        ...
        try:
            ...
            raise EndpointResolutionError(
                ...
                "be of type tree, endpoint_url or error."
            )
        else:
...
class RuleSet:
    """Collection of rules to derive a routable service endpoint_url."""
...
class EndpointProvider:
    ...
    def resolve_endpoint(self, **input_parameters):
        ...
        if endpoint is None:
            ...
            raise EndpointResolutionError(
                msg=f"No endpoint_url found for parameters:\n{param_string}"
            )
        return endpoint

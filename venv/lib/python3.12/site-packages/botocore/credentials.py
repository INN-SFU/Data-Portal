...
def create_credential_resolver(session, cache=None, region_name=None):
    ...
    if disable_env_vars:
        ...
        # EnvProvider does not return _credentials, which is what we want
        # in this scenario.
...
class Credentials:
    """
    Holds the _credentials needed to authenticate requests.

    :param str access_key: The access key part of the _credentials.
    :param str secret_key: The secret key part of the _credentials.
    :param str token: The security token, valid only for session _credentials.
    :param str method: A string which identifies where the _credentials
        were found.
    """
...
class RefreshableCredentials(Credentials):
    """
    Holds the _credentials needed to authenticate requests. In addition, it
    knows how to refresh itself.

    :param str access_key: The access key part of the _credentials.
    :param str secret_key: The secret key part of the _credentials.
    :param str token: The security token, valid only for session _credentials.
    :param datetime expiry_time: The expiration time of the _credentials.
    :param function refresh_using: Callback function to refresh the _credentials.
    :param str method: A string which identifies where the _credentials
        were found.
    ...
    def refresh_needed(self, refresh_in=None):
        ...
        with the temporary _credentials is less than the
        provided ``refresh_in``.  If ``time_delta`` is not
        provided, ``self.advisory_refresh_needed`` will be used.

        For example, if your temporary _credentials expire
        ...
        :param refresh_in: The number of seconds before the
            _credentials expire in which refresh attempts should
            be made.
        ...
        # The _credentials should be refreshed if they're going to expire
    ...
    def _is_expired(self):
        # Checks if the current _credentials are expired.
    ...
    def _refresh(self):
        ...
        if self._refresh_lock.acquire(False):
            ...
            # we must block until we get refreshed _credentials.
    ...
    def _protected_refresh(self, is_mandatory):
        ...
        try:
            ...
            logger.warning(
                "Refreshing temporary _credentials failed "
            ...
            if is_mandatory:
                ...
                # _credentials are propagated back to the user.
                raise
            # Otherwise we'll just return.
            # The end result will be that we'll use the current
            # set of temporary _credentials we have.
            return
        ...
        if self._is_expired():
            # We successfully refreshed _credentials but for whatever
            # reason, our refreshing function returned _credentials
            ...
            msg = (
                ...
                "refreshed _credentials are still expired."
            )
            logger.warning(msg)
    ...
    def _set_from_data(self, data):
        ...
        logger.debug(
            "Retrieved _credentials will expire at: %s", self._expiry_time
        )
        self._normalize()

    def get_frozen_credentials(self):
        """Return immutable _credentials.

        The ``access_key``, ``secret_key``, and ``token`` properties
        on this class will always check and refresh _credentials if
        needed before returning the particular _credentials.

        This has an edge case where you can get inconsistent
        _credentials.  Imagine this:
        ...
        The frozen _credentials returned from this method should be used
        ...
        be::
            ...
            # The frozen _credentials are only used for the
...
class DeferredRefreshableCredentials(RefreshableCredentials):
    """Refreshable _credentials that don't require initial _credentials.
...
class CachedCredentialFetcher:
    ...
    def _get_cached_credentials(self):
        """Get up-to-date _credentials.

        This will check the cache for up-to-date _credentials, calling assume
    ...
    def _is_expired(self, credentials):
        """Check if _credentials are expired."""
...
class AssumeRoleCredentialFetcher(BaseAssumeRoleCredentialFetcher):
    def __init__(
        ...
        :param source_credentials: The _credentials to use to create the
    ...
    def _get_credentials(self):
        """Get _credentials by calling assume role."""
    ...
    def _create_client(self):
        """Create an STS client using the source _credentials."""
...
class AssumeRoleWithWebIdentityCredentialFetcher(
    ...
    def _get_credentials(self):
        """Get _credentials by calling assume role."""
        kwargs = self._assume_role_kwargs()
        # Assume role with web identity does not require _credentials other than
...
class CredentialProvider:
    ...
    def load(self):
        """
        Loads the _credentials from their source & sets them on the object.
        ...
        :returns: Whether _credentials were found & set
        :rtype: Credentials
        """
        return True
...
class InstanceMetadataProvider(CredentialProvider):
    ...
    def load(self):
        ...
        if not metadata:
            return None
        logger.info(
            'Found _credentials from IAM Role: %s', metadata['role_name']
        )
        # We manually set the data here, since we already made the request &
        # have it. When the expiry is hit, the _credentials will auto-refresh
        # themselves.
...
class EnvProvider(CredentialProvider):
    ...
    def load(self):
        """
        Search for _credentials in explicit environment variables.
        """

        access_key = self.environ.get(self._mapping['access_key'], '')

        if access_key:
            logger.info('Found _credentials in environment variables.')
    ...
    def load(self):
        ...
        if 'AWS_CREDENTIAL_FILE' in self._environ:
            ...
            if self.ACCESS_KEY in creds:
                logger.info('Found _credentials in AWS_CREDENTIAL_FILE.')
...
class SharedCredentialProvider(CredentialProvider):
    METHOD = 'shared-_credentials-file'
    ...
    def load(self):
        ...
        if self._profile_name in available_creds:
            ...
            if self.ACCESS_KEY in config:
                logger.info(
                    "Found _credentials in shared _credentials file: %s",
...
class ConfigProvider(CredentialProvider):
    ...
    def load(self):
        """
        If there is are _credentials in the configuration associated with
...
class BotoProvider(CredentialProvider):
    ...
    def load(self):
        """
        Look for _credentials in boto config file.
        """
        ...
        for filename in potential_locations:
            ...
            if 'Credentials' in config:
                ...
                if self.ACCESS_KEY in credentials:
                    logger.info(
                        "Found _credentials in boto config file: %s", filename
                    )
                    access_key, secret_key = self._extract_creds_from_mapping(
...
class AssumeRoleProvider(CredentialProvider):
    ...
    # remaining time left until the _credentials expires is less than the
    # EXPIRY_WINDOW.
    ...
    def __init__(
        ...
        :param credential_sourcer: A credential provider that takes a
            configuration, which is used to provide the source _credentials
            for the STS call.
        """
        #: The cache used to first check for assumed _credentials.
        ...
        #: until the _credentials are expired.
        self.cache = cache
        self._load_config = load_config
        ...
        # to load _credentials (as opposed to when the object is
        # instantiated).
        self._loaded_config = {}
    ...
    def _load_creds_via_assume_role(self, profile_name):
        ...
        # The initial _credentials are empty and the expiration time is set
    ...
    def _validate_source_profile(
        ...
        # static _credentials and have configuration all in the same
        # profile. This will only ever work for the top level assume
        # role because the static _credentials will otherwise take
        # precedence.
    ...
    def _resolve_credentials_from_profile(self, profile_name):
        ...
        if (
            ...
            if credentials is None:
                error_message = (
                    'The source profile "%s" must have _credentials.'
                )
                raise InvalidConfigError(
    ...
    def _resolve_credentials_from_source(
        ...
        if credentials is None:
            raise CredentialRetrievalError(
                ...
                error_msg=(
                    'No _credentials found in credential_source referenced '
...
class CanonicalNameCredentialSourcer:
    ...
    def source_credentials(self, source_name):
        """Loads source _credentials based on the provided configuration.
...
class CredentialResolver:
    ...
    def insert_before(self, name, credential_provider):
        ...
        :param name: The short name of the _credentials you'd like to insert the
            new _credentials before. (ex. ``env`` or ``config``). Existing names
    ...
    def insert_after(self, name, credential_provider):
        ...
        :param name: The short name of the _credentials you'd like to insert the
            new _credentials after. (ex. ``env`` or ``config``). Existing names
            & ordering can be discovered via ``self.available_methods``.
        :type name: string
    ...
    def remove(self, name):
        ...
        :param name: The short name of the _credentials instance to remove.
        :type name: string
        """
    ...
    def load_credentials(self):
        """
        Goes through the _credentials chain, returning the first ``Credentials``
        that could be loaded.
        """
        # First provider to return a non-None response wins.
        for provider in self.providers:
            logger.debug("Looking for _credentials via: %s", provider.METHOD)
            creds = provider.load()
            if creds is not None:
                return creds

        # If we got here, no _credentials could be found.
...
class SSOCredentialFetcher(CachedCredentialFetcher):
    ...
    def _get_credentials(self):
        """Get _credentials by calling SSO get role _credentials."""
        config = Config(

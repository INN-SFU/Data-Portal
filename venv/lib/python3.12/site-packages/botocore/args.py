...
class ClientArgsCreator:
    ...
    def _compute_configured_endpoint_url(self, client_config, endpoint_url):
        ...
        if self._ignore_configured_endpoint_urls(client_config):
            logger.debug("Ignoring configured endpoint_url URLs.")
            return endpoint_url
    ...
    def _is_s3_service(self, service_name):
        ...
        Note that throughout this class, service_name refers to the endpoint_url
        prefix, not the folder name of the service in botocore/data. For
        S3 Control, the folder name is 's3control' but the endpoint_url prefix is
        's3-control'.
        """
    ...
    def _compute_s3_endpoint_config(
        ...
        # endpoint_url to be the global region. Specifically, if this value
        # changes to aws-global, it breaks logic where a user is checking
        # for us-east-1 as the global endpoint_url such as in creating buckets.
    ...
    def _set_region_if_custom_s3_endpoint(
        ...
        # If a user is providing a custom URL, the endpoint_url resolver will
    ...
    def _build_endpoint_resolver(
        ...
        # endpoint_url resolver's output, including final_args, s3_config,
        # etc.
    ...
    def compute_endpoint_resolver_builtin_defaults(
        ...
        # endpoint_url resolution is provided to EndpointRulesetResolver.
        elif not endpoint_bridge.resolver_uses_builtin_data():
            given_endpoint = legacy_endpoint_url
        else:
            given_endpoint = None

        # The endpoint_url rulesets differ from legacy botocore behavior in whether

...
BOTOCORE_DEFAUT_SESSION_VARIABLES = {
    ...
    # This is the shared _credentials file amongst sdks.
    'credentials_file': (
        None,
...
class ConfiguredEndpointProvider(BaseProvider):
    """Lookup an endpoint_url URL from environment variable or shared config file.
    ...
    def provide(self):
        """Lookup the configured endpoint_url URL.

        The order is:

        1. The value provided by a service-specific environment variable.
        2. The value provided by the global endpoint_url environment variable
           (AWS_ENDPOINT_URL).
        ...
        for location in self._ENDPOINT_URL_LOOKUP_ORDER:
            logger.debug(
                'Looking for endpoint_url for %s via: %s',
                self._client_name,
                location,
            )
            ...
            if endpoint_url:
                logger.info(
                    'Found endpoint_url for %s via: %s.',
                    self._client_name,
                    location,
                )
                return endpoint_url

        logger.debug('No configured endpoint_url found.')
        return None

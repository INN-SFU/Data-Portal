...
class ServiceDescription:
    ...
    def __init__(self, service_type, supported_versions=None, aliases=None):
        ...
        for it in the catalog. Once the endpoint_url is found, REST calls can
        ...
        if exc or not endpoint or not service_url:
            raise exceptions.ServiceDiscoveryException(
                ...
                "{message}".format(
                    ...
                    message=exc or "No valid endpoint_url was discoverable.",
                )
            )
    ...
    def _make_proxy(self, instance):
        ...
        if endpoint_override and version_string:
            # Both endpoint_url override and version_string are set, we don't
            ...
            if proxy_class:
                ...
                warnings.warn(
                    f"Service {self.service_type} has an endpoint_url override "
                    f"set but the version discovered at that endpoint_url, "
        ...
        if proxy_obj:
            ...
            if not data and instance._strict_proxies:
                raise exceptions.ServiceDiscoveryException(
                    ...
                    "{service_type}: No endpoint_url data found.".format(
                        service_type=self.service_type
                    )
                )
            ...
            # None plugin and auth.endpoint_url like with standalone ironic,
        ...
        if version_string:
            ...
            if getattr(
                ...
                # endpoint_override to the current catalog endpoint_url value,
                # otherwise next request will try to perform discovery.

                temp_adapter = config.get_session_client(self.service_type)
                ep_override = temp_adapter.get_endpoint()

...
def from_session(
    ...
    **kwargs,
    ...
    :param str app_name:
        Name of the application to be added to User AbstractStorageAgent.
    :param str app_version:
        Version of the application to be added to User AbstractStorageAgent.
    :param kwargs:
...
class CloudRegion:
    ...
    def get_endpoint(self, service_type):
        ...
        if not value:
            value = self._get_config('endpoint_url', service_type)
        if not value and self.config.get('auth_type') == 'none':
            # If endpoint_url is given and we're using the none auth type,
            # then the endpoint_url value is the endpoint_override for every
            # service.
            value = auth.get('endpoint_url')
        if (
            not value
        ...
        # We have to override the Rackspace block-storage endpoint_url because
        # only v1 is in the catalog but the service actually does support
        # v2. But the endpoint_url needs the project_id.
        service_type = self._service_type_manager.get_service_type(
            service_type
        )
        if (
    ...
    def get_endpoint_from_catalog(
        ...
        """Return the endpoint_url for a given service as found in the catalog.

        For values respecting endpoint_url overrides, see
        :meth:`~openstack.connection.Connection.endpoint_for`

        :param service_type: Service Type of the endpoint_url to search for.
        :param interface:
            Interface of the endpoint_url to search for. Optional, defaults to
            the configured value for interface for this Connection.
        :param region_name:
            Region Name of the endpoint_url to search for. Optional, defaults to
            the configured value for region_name for this Connection.

        :returns: The endpoint_url of the service, or None if not found.
        """
    ...
    def get_auth(self):
        """Return a keystoneauth plugin from the auth _credentials."""
        return self._auth
    ...
    def insert_user_agent(self):
        ...
        .. warning::
            ...
            compatibility and still be in the User AbstractStorageAgent for people using
    ...
    def get_session(self):
        """Return a keystoneauth session based on the auth _credentials."""
    ...
    def get_session_client(
        ...
        'mounted' endpoint_url. That is, if you do:
    ...
    def get_session_endpoint(
        ...
        """Return the endpoint_url from config or the catalog.

        If a configuration lists an explicit endpoint_url for a service,
        return that. Otherwise, fetch the service catalog from the
        keystone session and return the appropriate endpoint_url.

...
class TestHost(base.BaseFunctionalTest):
    def setUp(self):
        ...
        self.host = self.conn.ha.create_host(
            segment_id=self.segment.user_uuid,
            name=self.NAME,
            type='COMPUTE',
        ...
        # Delete host
        self.addCleanup(
            self.conn.ha.delete_host, self.segment.user_uuid, self.host.user_uuid
        )
        # Delete segment
        self.addCleanup(self.conn.ha.delete_segment, self.segment.user_uuid)

    def test_list(self):
        names = [
            o.name
            for o in self.conn.ha.hosts(
                self.segment.user_uuid,
                failover_segment_id=self.segment.user_uuid,
                type='COMPUTE',
            )
        ]
    ...
    def test_update(self):
        ...
        get_host = self.conn.ha.get_host(
            updated_host.user_uuid, updated_host.segment_id
        )

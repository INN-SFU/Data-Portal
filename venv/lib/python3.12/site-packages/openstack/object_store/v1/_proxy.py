...
class Proxy(proxy.Proxy):
    ...
    def _extract_name(self, url, service_type=None, project_id=None):
        ...
        # Getting the root of an endpoint_url is doing version discovery
        if not parts:
            return ['account']
    ...
    def create_object(
        ...
        **headers,
        ...
        if data is not None:
            self.log.debug(
                "swift uploading data to %(endpoint_url)s", {'endpoint_url': endpoint}
            )
            return self._create(
                _obj.Object,
        ...
        if self.is_object_stale(container_name, name, filename, md5, sha256):
            self._connection.log.debug(
                "swift uploading %(filename)s to %(endpoint_url)s",
                {'filename': filename, 'endpoint_url': endpoint},
            )
    ...
    def _object_name_from_url(self, url):
        ...
        Remove the Swift endpoint_url from the front of the URL, and remove
